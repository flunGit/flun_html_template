...
```json
{
	  "name": "lunjack-html-template", // 改为全局唯一名称
	  "version": "1.0.0",
...
>
	...
	## 安装
	```bash
	npm install lunjack-html-template
	```
	...
	### 作为模块使用
	```javascript
	const { startDevServer, compile } = require('lunjack-html-template');
...
```
	2.检查包名可用性:
		访问：https://www.npmjs.com/package/lunjack-html-template
		确保名称未被占用
>
	3.发布包:
```bash
...
```bash
		...
		# 链接包
			npm link lunjack-html-template
		...
		# 测试模块调用
			node -e "const pkg = require('lunjack-html-template'); pkg.startDevServer()"
```
---
...
```bash
		...
		# 测试安装(带详细输出)
			npm i lunjack-html-template-1.0.12.tgz --foreground-scripts
...
```
	2. 撤回发布（72小时内有效）
```bash
		npm unpublish lunjack-html-template@1.1.10  # 删除特定版本
		npm deprecate lunjack-html-template "此包已弃用，请勿再使用。" # 弃用包
		npm unpublish lunjack-html-template --force    # 删除整个包
```
---
